...
class OpenGLRenderer(
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        Log.d("KavachEngine", "GL ${GLES30.glGetString(GLES30.GL_VERSION)}")
    ...
    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        ...
        engine.inputManager.screenWidth  = width
        engine.inputManager.screenHeight = height
        engine.sceneManager.currentScene?.camera?.aspectRatio = width.toFloat() / height.toFloat()
        Log.d("KavachEngine", "Viewport ${width}×${height}")
    }
